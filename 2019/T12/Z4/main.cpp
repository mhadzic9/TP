...
public:
	...
	friend Kompleksni operator *(const Kompleksni &a,const Kompleksni &b)
	{
		return {a.re*b.re-a.im*b.im,a.re*b.re+a.im*b.im};
	}
	friend Kompleksni operator /(const Kompleksni &a,const Kompleksni &b)
	{
	...
	{
		if (a.re!=0 && a.im>0 && a.im!=1) tok << a.re << "+" << a.im << "i";
		else if (a.im<0 && a.im!=1 && a.re!=0 && a.im!=-1) tok << a.re << a.im << "i";
		else if (a.im==1 && a.re!=0) tok << a.re << "+i";
		else if (a.im==-1 && a.re!=0) tok << a.re << "-i";
		else if (a.re==0) {
			if (a.im==1) tok << "i";
			else if (a.im==-1) tok << "-i";
			else if (a.im==0) tok << a.re;
			else tok << a.im << "i";
		} else if (a.im==0) tok << a.re;
		return tok;
	}
