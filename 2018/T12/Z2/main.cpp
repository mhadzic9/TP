...
std::set<std::pair<Tip,Tip2>> operator%(const std::set<Tip> &s1,const std::set<Tip2> &s2)
{
	std::set<std::pair<Tip,Tip2>>pom;
	for(auto it=s1.begin(); it!=s1.end(); it++)
		for(auto it2=s2.begin(); it2!=s2.end(); it2++)
			pom.insert(std::pair<Tip,Tip2>(*it,*it2));
	return pom;
}
template<typename Tip>
...
template <typename Tip,typename Tip2>
std::ostream &operator << (std::ostream & ispisi, std::set<std::pair<Tip,Tip2>> s1) {
	ispisi << "{";
	for(auto it=s1.begin();it!=s1.end();it++) {
